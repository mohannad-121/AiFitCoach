...
type AdminUserDetail = {
  ...
  notes?: Array<{
    ...
    created_at?: string | null;
    pinned_exercise?: { name?: string; nameAr?: string } | null;
  }>;
};
...
export function AdminPage() {
  ...
  const [noteForm, setNoteForm] = useState({ note_text: '', note_category: 'general', related_date: '' });
  const [pinnedExercise, setPinnedExercise] = useState<{ name: string; nameAr: string } | null>(null);
  const [planExercises, setPlanExercises] = useState<Array<{ label: string; name: string; nameAr: string }>>([]);
  ...
  const fetchUserPlansForPicker = async (activeSession: AdminSession, userId: string) => {
    setLoadingPlans(true);
    setPlanExercises([]);
    setPinnedExercise(null);
    try {
      const response = await fetch(`${AI_BACKEND_URL}/admin/users/${encodeURIComponent(userId)}/plans`, {
        headers: adminHeaders(activeSession),
      });
      if (!response.ok) return;
      const payload = await response.json().catch(() => null);
      const plans: Array<{ title?: string; plan_data?: Array<{ day?: string; dayAr?: string; exercises?: Array<{ name: string; nameAr?: string }> }> }> = Array.isArray(payload?.plans) ? payload.plans : [];
      const exercises: Array<{ label: string; name: string; nameAr: string }> = [];
      const seen = new Set<string>();
      for (const plan of plans) {
        const planDays = Array.isArray(plan.plan_data) ? plan.plan_data : [];
        for (const day of planDays) {
          ...
          for (const ex of (Array.isArray(day.exercises) ? day.exercises : [])) {
            ...
            seen.add(key);
            exercises.push({
              label: dayLabel ? `${dayLabel} — ${ex.name}` : ex.name,
              name: ex.name,
              nameAr: ex.nameAr || ex.name,
            });
          }
        }
      }
    ...
    try {
      const response = await fetch(`${AI_BACKEND_URL}/admin/users/${encodeURIComponent(selectedUserId)}/notes`, {
        ...
        body: JSON.stringify({
          ...
          ...(pinnedExercise ? { pinned_exercise: pinnedExercise } : {}),
        }),
      });
      const payload = await response.json().catch(() => null);
      if (response.status === 403) {
      ...
      setNoteForm({ note_text: '', note_category: noteForm.note_category, related_date: '' });
      setPinnedExercise(null);
      toast({
  ...
  useEffect(() => {
    if (noteForm.note_category === 'workout' && session && selectedUserId) {
      fetchUserPlansForPicker(session, selectedUserId);
    } else {
      setPlanExercises([]);
      setPinnedExercise(null);
    }
  }, [noteForm.note_category, selectedUserId]);
                          ...
                          <div className="rounded-3xl border border-border/60 bg-secondary/15 p-6">
                            ...
                            {detail.notes?.[0] ? (
                              <div className="rounded-2xl border border-border/60 bg-secondary/20 p-4">
                                ...
                                <p className="text-sm text-foreground whitespace-pre-wrap">{detail.notes[0].note_text}</p>
                              </div>
                            ) : (
                              <div className="rounded-2xl border border-dashed border-border/60 p-4 text-sm text-muted-foreground">
                      ...
                      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(320px,0.8fr)_minmax(0,1fr)]">
                        ...
                        <div className="space-y-3">
                          <select
                            ...
                            onChange={(event) => setNoteForm((current) => ({ ...current, note_category: event.target.value }))}
                            className="w-full rounded-xl border border-border/60 bg-background px-4 py-3 outline-none focus:ring-2 focus:ring-primary/40"
                          >
                          ...
                          {noteForm.note_category === 'workout' && (
                            <div>
                              <label className="text-xs text-muted-foreground block mb-1">
                                {language === 'ar' ? 'اختر تمريناً من الخطة النشطة (اختياري)' : 'Pick an exercise from the active plan (optional)'}
                              </label>
                              <select
                                value={pinnedExercise?.name || ''}
                                onChange={(event) => {
                                  const selected = planExercises.find((e) => e.name === event.target.value);
                                  setPinnedExercise(selected ? { name: selected.name, nameAr: selected.nameAr } : null);
                                }}
                                disabled={loadingPlans}
                                className="w-full rounded-xl border border-border/60 bg-background px-4 py-3 outline-none focus:ring-2 focus:ring-primary/40 disabled:opacity-50"
                              >
                                <option value="">
                                  {loadingPlans
                                    ? (language === 'ar' ? 'جار تحميل التمارين...' : 'Loading exercises...')
                                    : (language === 'ar' ? '— لا يوجد تمرين محدد —' : '— No specific exercise —')}
                                </option>
                              ...
                              {pinnedExercise && (
                                <p className="mt-1 text-xs text-primary">
                                  {language === 'ar' ? `سيتم إشعار المستخدم بـ: ${pinnedExercise.nameAr || pinnedExercise.name}` : `User will be notified about: ${pinnedExercise.name}`}
                                </p>
                              )}
                            </div>
                          )}
                            ...
                            {(detail.notes || []).map((note) => (
                              <div key={note.id} className="rounded-2xl border border-border/60 bg-secondary/20 p-4">
                                ...
                                <div className="flex items-center gap-2 mb-2">
                                  <span className="text-xs text-primary uppercase tracking-wide">{note.note_category || 'general'}{note.related_date ? ` • ${note.related_date}` : ''}</span>
                                  {note.pinned_exercise?.name && (
                                    <span className="inline-flex items-center gap-1 rounded-full bg-primary/15 px-2 py-0.5 text-[10px] font-semibold text-primary">
                                      📌 {language === 'ar' ? note.pinned_exercise.nameAr || note.pinned_exercise.name : note.pinned_exercise.name}
                                    </span>
                                  )}
                                </div>
