"""
Train NEW datasets (completely different from the 11 already trained).
Focus on activity, nutrition, and exercise datasets not yet used.
"""
...
# NEW DATASETS (not trained before)
...
def _minute_calories_narrow_band(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series, dict[str, Any]]:
    """Minute-level calorie classification."""
    work = df.copy()
    if "Calories" in work.columns:
        cal_band = _quantile_target(work["Calories"])
        feature_columns = [col for col in work.columns if col != "Calories"][:40]
        return work[feature_columns], pd.Series(cal_band, name="minute_cal_band"), {
            "target": "derived_minute_calorie_band",
            "notes": "3-class minute-level calorie band.",
        }
    raise ValueError("Calories column not found")


def _minute_steps_narrow_band(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series, dict[str, Any]]:
    """Minute-level steps classification."""
    work = df.copy()
    if "Steps" in work.columns:
        steps_band = _quantile_target(work["Steps"])
        feature_columns = [col for col in work.columns if col != "Steps"][:40]
        return work[feature_columns], pd.Series(steps_band, name="minute_steps_band"), {
            "target": "derived_minute_steps_band",
            "notes": "3-class minute-level steps band.",
        }
...
def _heart_rate_band(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series, dict[str, Any]]:
    ...
    if "Value" in work.columns:
        ...
        feature_columns = [col for col in work.columns if col != "Value"]
...
def _minute_mets_band(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series, dict[str, Any]]:
    ...
    if "METs" in work.columns:
        ...
        feature_columns = [col for col in work.columns if col != "METs"][:40]
...
def _minute_sleep_band(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series, dict[str, Any]]:
    ...
    if "value" in work.columns:
        ...
        feature_columns = [col for col in work.columns if col != "value"][:40]
...
if __name__ == "__main__":
    ...
    for item in benchmark["results"]:
        ...
        print(f"  Rows Used: {item['rows_used']} | Features: {item['feature_count']} | Classes: {item['class_count']}")
        print(f"  >>> ACCURACY: {metrics['accuracy']:.4f} | F1 (weighted): {metrics['weighted_f1']:.4f}")
